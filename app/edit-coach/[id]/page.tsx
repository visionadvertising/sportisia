...
export default function EditCoach() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        success('Antrenorul a fost actualizat cu succes!');
        setTimeout(() => {
          router.push(`/coach/${params.id}`);
        }, 500);
      } else {
        const errorData = await response.json().catch(() => ({}));
        showError(errorData.error || 'Eroare la actualizarea antrenorului');
      }
    } catch (error) {
      console.error('Error updating coach:', error);
      showError('Eroare la actualizarea antrenorului. Verifică conexiunea la internet.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div>
      <div style={{
        ...
        <div className="container">
          <Link href={`/coach/${params.id}`} style={{ color: 'white', marginBottom: '16px', display: 'inline-block', opacity: 0.9 }}>
            ← Înapoi
          </Link>
          <h1 style={{ fontSize: '2.5rem', fontWeight: 'bold' }}>Editează Antrenor</h1>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, sport: e.target.value as Coach['sport'] })}
                >
              ...
              <div style={{ display: 'flex', gap: '12px', marginTop: '30px' }}>
                <Link href={`/coach/${params.id}`} className="btn btn-secondary" style={{ flex: 1 }}>
                  Anulează
                </Link>
                <button
                  type="submit"
