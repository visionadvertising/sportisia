...
export default function CitySportCoachesPage() {
  ...
  const fetchCoaches = async () => {
    try {
      ...
      const filtered = data.filter((coach: Coach) =>
        coach.city === city && coach.sport === sport
      );
      setCoaches(filtered);
    } catch (error) {
      ...
      <div style={{
        ...
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <h1 style={{
            ...
            Antrenori {getSportLabel(sport)} în {city}
          </h1>
          <p style={{
            fontSize: '1.2rem',
            opacity: 0.95,
            ...
            Găsește antrenori de {getSportLabel(sport).toLowerCase()} disponibili pentru rezervare
          </p>
        </div>
      </div>
        ...
        <div style={{ marginBottom: '30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            <h2 style={{ fontSize: '1.5rem', fontWeight: '600', color: '#111827', marginBottom: '8px' }}>
              {sortedCoaches.length} antrenor{sortedCoaches.length !== 1 ? 'i' : ''} disponibil{sortedCoaches.length !== 1 ? 'i' : ''}
            </h2>
          </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            <p style={{ fontSize: '18px', color: '#6b7280', marginBottom: '20px' }}>
              Nu există antrenori de {getSportLabel(sport).toLowerCase()} disponibili în {city} momentan.
            </p>
            <Link href="/add-coach" className="btn btn-primary" style={{ background: '#0d9488' }}>
