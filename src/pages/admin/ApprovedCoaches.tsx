...
function ApprovedCoaches() {
  ...
  const loadFacilities = async () => {
    setLoading(true)
    try {
      const queryParams = new URLSearchParams({ status: 'active', type: 'coach' })
  ...
  const loadSports = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        setAvailableSports([...new Set(['tenis', 'fotbal', 'baschet', 'volei', 'handbal', 'badminton', 'squash', ...sports])].sort())
      }
    } catch (err) {
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {facility.specialization && (
                      <div style={{ color: '#64748b', fontSize: '0.875rem' }}>
                        <strong>Specializare:</strong> {facility.specialization}
                      </div>
                    )}
                  </div>
                  <Link
