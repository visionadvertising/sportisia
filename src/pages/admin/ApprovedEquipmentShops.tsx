...
function ApprovedEquipmentShops() {
  ...
  const loadFacilities = async () => {
    setLoading(true)
    try {
      const queryParams = new URLSearchParams({ status: 'active', type: 'equipment_shop' })
  ...
  const loadSports = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        setAvailableSports([...new Set(['general', 'tenis', 'fotbal', 'baschet', 'volei', 'handbal', 'badminton', 'squash', ...sports])].sort())
      }
    } catch (err) {
      console.error('Error loading sports:', err)
      setAvailableSports(['general', 'tenis', 'fotbal', 'baschet', 'volei', 'handbal', 'badminton', 'squash'])
    }
  }
            ...
            >
              ...
              {availableSports.map(sport => (
                <option key={sport} value={sport}>
                  {sport === 'general' ? 'General (toate sporturile)' : sport.charAt(0).toUpperCase() + sport.slice(1)}
                </option>
              ))}
            </select>
          </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '3rem' }}>
            <p style={{ color: '#64748b', fontSize: '1.125rem' }}>
              Nu există magazine de articole aprobate.
            </p>
          </div>
        ) : (
          <>
...
export default ApprovedEquipmentShops
