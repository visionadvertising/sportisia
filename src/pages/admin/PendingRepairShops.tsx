...
function PendingRepairShops() {
  ...
  const handleApproveFacility = async (facilityId: number) => {
    try {
      ...
      const data = await response.json()
      if (data.success) {
        setSuccess('Magazinul de reparații a fost aprobat cu succes')
  ...
  const handleRejectFacility = async (facilityId: number) => {
    try {
      ...
      const data = await response.json()
      if (data.success) {
        setSuccess('Magazinul de reparații a fost respins')
        setTimeout(() => setSuccess(''), 3000)
        loadFacilities()
      } else {
    ...
    <AdminLayout>
      <div style={{ padding: '2rem' }}>
        <h1 style={{ fontSize: '2rem', fontWeight: '600', marginBottom: '2rem', color: '#0f172a' }}>
          Magazine Reparații în Așteptare
        </h1>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '3rem' }}>
            <p style={{ color: '#64748b', fontSize: '1.125rem' }}>
              Nu există magazine de reparații în așteptare.
            </p>
          </div>
        ) : (
          <>
...
export default PendingRepairShops
