...
export function generateDescription(
  ...
  if (filters.type === 'field' && filters.sport) {
    ...
    const cityPart = filters.city ? `/${cityNameToSlug(filters.city)}` : ''
    const cityText = filters.city ? ` din ${filters.city}` : ''
    paragraphs.push(`<p>Căutând antrenori pentru ${sportName}? Explorează <a href="${cityPart}/${sportSlug}/antrenori" style="color: #10b981; text-decoration: underline;">antrenorii de ${sportName}${cityText}</a> disponibili pe platformă. Un antrenor specializat poate face diferența în dezvoltarea tehnicii și performanțelor tale. Fie că ești începător sau avansat, găsești antrenori care se adaptează nivelului tău și îți oferă programe personalizate de antrenament. De asemenea, dacă ai nevoie de echipamente sau servicii de întreținere, explorează <a href="${cityPart || ''}/magazine-articole" style="color: #10b981; text-decoration: underline;">magazinele de articole sportive</a> și <a href="${cityPart || ''}/magazine-reparatii" style="color: #10b981; text-decoration: underline;">serviciile de reparații</a> disponibile în zona ta.</p>`)
  } else if (filters.type === 'coach' && filters.sport) {
    const sportName = SPORT_NAMES[filters.sport] || filters.sport
    const sportSlug = filters.sport
    const cityPart = filters.city ? `/${cityNameToSlug(filters.city)}` : ''
    const cityText = filters.city ? ` din ${filters.city}` : ''
    paragraphs.push(`<p>Ai nevoie de un teren pentru ${sportName}? Vezi <a href="${cityPart}/${sportSlug}/terenuri" style="color: #10b981; text-decoration: underline;">terenurile de ${sportName}${cityText}</a> disponibile pe platformă. Fiecare teren este prezentat cu detalii despre suprafață, echipamente disponibile și facilități oferite. De asemenea, pentru echipamente și accesorii, explorează <a href="${cityPart || ''}/magazine-articole" style="color: #10b981; text-decoration: underline;">magazinele de articole sportive</a> din zona ta, unde găsești tot ce ai nevoie pentru practicarea ${sportName}. Pentru întreținerea echipamentelor, <a href="${cityPart || ''}/magazine-reparatii" style="color: #10b981; text-decoration: underline;">serviciile de reparații</a> sunt disponibile pentru a-ți menține echipamentele în stare optimă.</p>`)
  } else if (filters.type === 'field' && filters.city) {
    paragraphs.push(`<p>Căutând antrenori în ${filters.city}? Explorează <a href="${cityNameToSlug(filters.city)}/antrenori" style="color: #10b981; text-decoration: underline;">antrenorii disponibili</a> din oraș, care oferă programe personalizate pentru diverse sporturi. Fie că ești interesat de tenis, fotbal, baschet sau alte activități, găsești antrenori specializați care îți pot ajuta să-ți atingi obiectivele. Pentru echipamente și accesorii, <a href="${cityNameToSlug(filters.city)}/magazine-articole" style="color: #10b981; text-decoration: underline;">magazinele de articole sportive</a> din ${filters.city} oferă o gamă variată de produse. De asemenea, dacă ai nevoie de servicii de întreținere, <a href="${cityNameToSlug(filters.city)}/magazine-reparatii" style="color: #10b981; text-decoration: underline;">magazinele de reparații</a> sunt disponibile pentru a-ți menține echipamentele în stare optimă.</p>`)
  } else if (filters.type === 'coach' && filters.city) {
    paragraphs.push(`<p>Ai nevoie de un teren în ${filters.city}? Vezi <a href="${cityNameToSlug(filters.city)}/terenuri" style="color: #10b981; text-decoration: underline;">terenurile disponibile</a> din oraș, fiecare cu caracteristici unice și servicii specializate. Fie că cauți un teren de tenis, fotbal, baschet sau alt sport, ${filters.city} oferă opțiuni diverse. Pentru echipamente și accesorii, explorează <a href="${cityNameToSlug(filters.city)}/magazine-articole" style="color: #10b981; text-decoration: underline;">magazinele de articole sportive</a> din oraș. De asemenea, pentru întreținerea echipamentelor, <a href="${cityNameToSlug(filters.city)}/magazine-reparatii" style="color: #10b981; text-decoration: underline;">serviciile de reparații</a> sunt disponibile pentru a-ți menține echipamentele în stare optimă și a prelungi durata de viață a acestora.</p>`)
  } else if (filters.sport && !filters.type) {
    const sportName = SPORT_NAMES[filters.sport] || filters.sport
    const sportSlug = filters.sport
